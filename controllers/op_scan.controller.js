...
function forbid(res, msg, actor) {
  return res.status(403).json({ message: msg, actor });
}
...
async function genNewLot(conn, tk_id, part_id, actor_u_id) {
  ...
  const [[outRow]] = await conn.query(
    `SELECT @run_no AS run_no, @lot_no AS lot_no`
  );
...
function rebuildMasterLotNo(sourceLotNo, outPartNo, outPartName) {
  ...
  const lastDash = lot.lastIndexOf("-");

  if (firstDash <= 0 || lastDash <= firstDash) {
    throw new Error(`Invalid lot format for Master-ID rebuild: ${lot}`);
  }

  const lotDate = lot.slice(0, firstDash).trim();
  const runNo = lot.slice(lastDash + 1).trim();
...
exports.startOpScan = async (req, res) => {
    ...
    try {
      ...
      if (Number(headRow?.tk_active) !== 1) {
        ...
        return res.status(403).json({
          ...
          actor,
          tk_id,
          tk_active: Number(headRow?.tk_active),
        });
      }
      ...
      if (headStatus === 1) {
        await conn.rollback(); conn.release();
        return res.status(403).json({
          message: "Tracking No. นี้ดำเนินการเสร็จสิ้นแล้ว ไม่สามารถเริ่มงานได้",
          actor,
          tk_id,
        });
      }
      ...
      if (!tkDoc) {
        await conn.rollback(); conn.release();
        return res.status(404).json({
          message: "ไม่พบข้อมูล TKDetail ของ Tracking No. นี้",
          actor,
          tk_id,
        });
      }
      ...
      const selfRefLots = new Set(
        tRows
          .filter(r =>
            (r.lot_parked_status === 0 || r.lot_parked_status === false) &&
      ...
      if (finishedRows[0]) {
        await conn.rollback(); conn.release();
        return res.status(403).json({
          message: "Already FINISHED at STA007.",
          actor,
          tk_id,
        });
      }
        ...
        if (Number.isFinite(lastNum) && Number.isFinite(currentNum) && currentNum <= lastNum) {
          const [nextStaRows] = await conn.query(
            `SELECT op_sta_id, op_sta_name
             FROM \`op_station\`
             WHERE CAST(REPLACE(op_sta_id,'STA','') AS UNSIGNED) > ?
               AND op_sta_active = 1
      ...
      if (!mcRow) {
        ...
        return res.status(400).json({
          message: "ไม่พบ Machine หรือ Machine ถูกปิดใช้งาน",
          actor,
          MC_id,
        });
      }
      ...
      if (activeRows[0]) {
        ...
        if (activeUId === actorUId) {
          ...
          return res.status(200).json({
            message: "Lot No. นี้กำลังทำอยู่ในหน้า Active Scan",
            actor,
            tk_id,
        ...
        await conn.rollback(); conn.release();
        const ownerName =
          [activeScan.u_firstname, activeScan.u_lastname].filter(Boolean).join(" ") ||
          `u_id ${activeUId}`;
      ...
      return res.status(201).json({
        ...
        actor: {
          u_id: actor.u_id,
          u_firstname: actor.u_firstname,
          u_lastname: actor.u_lastname,
          role: actor.role,
        },
        op_sc_id,
        op_sta_id,
  ...
  lot_no:     base_lot_no || tkDoc.lot_no || null,
        ...
        current_lots: leafLotSet.length > 0
          ...
          : base_lot_no
            ? [{
                run_no: base_run_no,
                lot_no: base_lot_no,
                part_no: tkDoc.part_no ?? null,
                part_name: tkDoc.part_name ?? null,
              }]
            : [],
      });
    } catch (e) {
...
exports.finishOpScan = async (req, res) => {
  ...
  if (!op_sc_id) {
    return res.status(400).json({ message: "op_sc_id is required", actor });
  }
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    if (![1, 2, 3].includes(tf)) {
      return res.status(400).json({
        message: `groups[${num}]: tf_rs_code must be 1, 2, or 3`,
        actor,
      });
    }
    ...
    if (tf === 1) {
      if (!Number.isFinite(Number(g.qty)) || Number(g.qty) <= 0) {
        return res.status(400).json({
          message: `groups[${num}]: qty is required for tf=1`,
          actor,
        });
      }
      if (!String(g.out_part_no || "").trim()) {
        return res.status(400).json({
          message: `groups[${num}]: out_part_no is required`,
          actor,
        });
      }
    }
  ...
  if (!Number.isFinite(Number(g.qty)) || Number(g.qty) <= 0) {
    return res.status(400).json({
      message: `groups[${num}]: qty is required for tf=2`,
      actor,
    });
  }
  ...
  if (!String(g.from_lot_no || "").trim()) {
    return res.status(400).json({
      message: `Group ${num}: กรุณาเลือก From Lot ก่อนบันทึก (Split-ID ต้องระบุ Lot ต้นทาง)`,
      actor,
    });
  }
  ...
  for (const s of g.splits) {
    if (!String(s.out_part_no || "").trim()) {
      return res.status(400).json({
        message: `groups[${num}] split: out_part_no is required`,
        actor,
      });
    }
    if (!Number.isFinite(Number(s.qty)) || Number(s.qty) <= 0) {
      return res.status(400).json({
        message: `groups[${num}] split: qty must be > 0`,
        actor,
      });
    }
  ...
  if (sumSplit > Math.trunc(Number(g.qty))) {
    return res.status(400).json({
      message: `groups[${num}]: sum splits (${sumSplit}) เกิน group qty (${Math.trunc(Number(g.qty))})`,
      actor,
    });
  }
}
    ...
    if (tf === 3) {
      ...
      if (!Array.isArray(g.merge_lots) || g.merge_lots.length < 2) {
        return res.status(400).json({
          message: `groups[${num}]: merge_lots[] must have >= 2 lots`,
          actor,
        });
      }
      ...
      for (const m of g.merge_lots) {
        if (!String(m.from_lot_no || "").trim()) {
          return res.status(400).json({
            message: `groups[${num}]: every merge_lot must have from_lot_no`,
            actor,
          });
        }
        if (!Number.isFinite(Number(m.qty)) || Number(m.qty) <= 0) {
          return res.status(400).json({
            message: `groups[${num}]: merge_lot qty must be > 0`,
            actor,
          });
        }
      }
  ...
  for (const g of groups) {
    ...
    let lots = [];
    if (tf === 1 || tf === 2) {
      lots = g.from_lot_no ? [String(g.from_lot_no).trim()] : [];
    } else if (tf === 3) {
      lots = Array.isArray(g.merge_lots)
        ? g.merge_lots.map(m => String(m.from_lot_no || "").trim()).filter(Boolean)
        : [];
    }
    ...
    for (const lot of lots) {
      ...
      if (allFromLots.has(lot)) {
        return res.status(400).json({
          message: `from_lot_no "${lot}" ใช้ซ้ำในหลาย group`,
          actor,
        });
      }
      allFromLots.add(lot);
  ...
  try {
    ...
    try {
      ...
      if (!actorSta) {
        await conn.rollback(); conn.release();
        return res.status(400).json({
          message: "Missing op_sta_id in token",
          actor,
        });
      }
      ...
      if (scanSta && scanSta !== actorSta) {
        await conn.rollback(); conn.release();
        return res.status(403).json({
          message: `Scan started at ${scanSta} but you login as ${actorSta}`,
          actor,
          op_sc_id,
        });
      }
      ...
      if (Number(headRows[0]?.tk_active) !== 1) {
        ...
        return res.status(403).json({
          ...
          actor,
          tk_id: master_tk_id,
          tk_active: Number(headRows[0]?.tk_active),
        });
      }
      ...
      if (Number(headRows[0]?.tk_status) === 1) {
        await conn.rollback(); conn.release();
        return res.status(403).json({
          message: "This tk_id is FINISHED. Cannot finish again.",
          actor,
          tk_id: master_tk_id,
        });
      }
...
// ⑤ validate from_lot_no + parked lot station check
{
  ...
  for (const g of groups) {
    ...
    if (tf === 1) {
      const lot = g.from_lot_no
        ? String(g.from_lot_no).trim()
        : base_lot_no;
      if (lot) fromLotChecks.push({ lot, gLabel: "tf=1", tf });
    }
    ...
    if (tf === 2) {
      const lot = g.from_lot_no
        ? String(g.from_lot_no).trim()
        : null;
      if (lot) fromLotChecks.push({ lot, gLabel: "tf=2", tf });
    }

    if (tf === 3) {
      for (const m of g.merge_lots) {
        const lot = String(m.from_lot_no || "").trim();
        if (lot) fromLotChecks.push({ lot, gLabel: "tf=3 merge_lot", tf });
      }
    }
  }
  ...
  for (const { lot, gLabel, tf } of fromLotChecks) {
    ...
    if (!lotOwnerTk && tf === 3) {
      const [tfRows] = await conn.query(
        `SELECT from_tk_id, to_tk_id
         FROM ${SAFE_TRANSFER}
         WHERE to_lot_no = ? OR from_lot_no = ?
      ...
      if (tfRows[0]) {
        lotOwnerTk =
          tfRows[0].to_tk_id
            ? String(tfRows[0].to_tk_id).trim()
            : tfRows[0].from_tk_id
              ? String(tfRows[0].from_tk_id).trim()
              : null;
      }
    }
    ...
    if (!lotOwnerTk) {
      await conn.rollback(); conn.release();
      return res.status(400).json({
        message: `${gLabel}: from_lot_no "${lot}" ไม่พบใน TKRunLog`,
        actor,
      });
    }
    ...
    const [parkedRows] = await conn.query(
      `SELECT op_sta_id, lot_parked_status
       FROM ${SAFE_TRANSFER}
       WHERE to_lot_no = ?
       ORDER BY transfer_ts DESC, transfer_id DESC
       LIMIT 1`,
      [lot]
    );
...
if (tf === 1) {
  ...
  if (!outPart) {
    await conn.rollback(); conn.release();
    return res.status(400).json({
      message: `groups[${gNum}]: out_part_no "${g.out_part_no}" not found`,
      actor,
    });
  }

  const from_lot = g.from_lot_no
    ? String(g.from_lot_no).trim()
    : base_lot_no;
  ...
  if (!from_lot) {
    await conn.rollback(); conn.release();
    return res.status(400).json({
      message: `groups[${gNum}]: ไม่พบ from_lot_no สำหรับ Master-ID`,
      actor,
    });
  }
  ...
  if (!first_lot_no) first_lot_no = to_lot;
...
if (tf === 2) {
  ...
  const [srcRunRows] = await conn.query(
    `SELECT run_no
     ...
     WHERE tk_id = ? AND lot_no = ?
     LIMIT 1`,
    [fromOwnerTk_2, from_lot]
  );
  const sourceRunNo = srcRunRows[0]?.run_no
    ? String(srcRunRows[0].run_no).trim()
    : null;

  if (!sourceRunNo) {
    await conn.rollback(); conn.release();
    return res.status(400).json({
      message: `groups[${gNum}]: run_no ของ from_lot "${from_lot}" not found`,
      actor,
    });
  }
  ...
  const sourceMiddle =
    sourceFirstDash >= 0 && sourceLastDash > sourceFirstDash
      ? from_lot.slice(sourceFirstDash + 1, sourceLastDash).trim()
      : "";

  let sourcePartNo = null;
  ...
  if (sourceMiddle) {
    const [partRows] = await conn.query(
      `SELECT part_id, part_no, part_name
       FROM ${SAFE_PART}
       WHERE CONCAT(TRIM(part_no), '-', TRIM(part_name)) = ?
       LIMIT 1`,
      [sourceMiddle]
    );

    if (partRows[0]) {
      sourcePartId = partRows[0].part_id ?? null;
      sourcePartNo = partRows[0].part_no ?? null;
      sourcePartName = partRows[0].part_name ?? null;
    }
  }
  ...
  if (!sourcePartId) {
    ...
    const srcDetail = srcDetailRows[0] ?? null;
    sourcePartId = srcDetail?.part_id ?? null;
    sourcePartNo = srcDetail?.part_no ?? null;
    sourcePartName = srcDetail?.part_name ?? null;
  }
  ...
  for (let si = 0; si < splits.length; si++) {
    ...
    if (!outPart) {
      await conn.rollback(); conn.release();
      return res.status(400).json({
        message: `groups[${gNum}] split: out_part_no "${s.out_part_no}" not found`,
        actor,
      });
    }
  ...
  if (parkedQty > 0) {
    ...
    if (!sourcePartId) {
      await conn.rollback(); conn.release();
      return res.status(400).json({
        message: `groups[${gNum}]: cannot resolve current source part for parked lot`,
        actor,
      });
    }

    const { run_no: p_run, lot_no: p_lot } = await genNewLot(
      conn,
      master_tk_id,
      Number(sourcePartId),
      actor.u_id
    ...
    parkedLots.push({
      ...
      lot_no: p_lot,
      qty: parkedQty,
      parked_at_sta: actorSta,
    });
  }

  const latestLot = splitLots.length
    ? splitLots[splitLots.length - 1].lot_no
    : parkedLots.length
        ? parkedLots[0].lot_no
        : null;
  ...
  created_children.push({
    ...
    parked_lots: parkedLots,
        ...
        if (tf === 3) {
          ...
          const { run_no, lot_no } = await genNewLot(
            conn,
            master_tk_id,
            outPart.part_id,
            actor.u_id
          ...
          for (const m of g.merge_lots) {
            ...
            await insertTransfer(conn, {
              from_tk_id: mergeOwnerTk,
              to_tk_id: master_tk_id,
              from_lot_no: mLotNo,
              to_lot_no: lot_no,
              tf_rs_code: 3,
              transfer_qty: Math.trunc(Number(m.qty)),
              op_sc_id,
              MC_id: MC_id_val,
              op_sta_id: actorSta,
              lot_parked_status: 0,
          ...
          if (Array.isArray(g.parked_from_lots) && g.parked_from_lots.length > 0) {
            for (const pl of g.parked_from_lots) {
              ...
              const pPartId =
                await getPartIdByLotNo(conn, plOwnerTk, pLotNo) ||
                await getPartIdByLotNo(conn, master_tk_id, pLotNo);

              if (!pPartId) {
                await conn.rollback(); conn.release();
                return res.status(400).json({
                  message: `parked lot "${pLotNo}" part_id not found in TKRunLog`,
                  actor,
                });
              }
          ...
          created_children.push({
            ...
            to_lot_no: lot_no,
            merge_qty: g.qty,
            merged_from: g.merge_lots.map(m => ({
              from_lot_no: m.from_lot_no,
              qty: Math.trunc(Number(m.qty)),
            })),
           ...
           WHERE (t.from_tk_id = ? OR t.to_tk_id = ?)
             ...
             AND NOT EXISTS (
               ...
               WHERE t2.from_lot_no = t.to_lot_no
                 ...
                 /* ลบ tf_rs_code = 3 ออก เพราะต้องกัน Split (tf=2) ด้วย
                    เงื่อนไข to_lot_no != from_lot_no ครอบคลุม tf=2,3 อยู่แล้ว
                    Master (tf=1) มี from==to เสมอ → ไม่กระทบ
                    กรณีที่แก้: lot ที่ถูก Split ที่ station ก่อน → ไม่ควร self-park
                    ที่ station ถัดไป เช่น K26-MB-000045 ถูก Split ที่ STA006
                    → ไม่ควรขึ้นเป็น "ไม่ได้ใช้" ที่ STA007 */
  ...
  color_id: lr.color_id ?? null,
      ...
      await conn.query(
        `UPDATE ${SAFE_OPSCAN}
         ...
         WHERE op_sc_id = ?`,
        [
          Math.trunc(total_qty),
          Math.trunc(good_qty),
          Math.trunc(scrap_qty),
